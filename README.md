...
| 串口数据需要实时计算校验值 | **CRC 助手**，5 种算法实时计算，支持追加到发送区 |
| AT 指令调试需反复输入相同指令 | **AT 指令伴侣**，模板库一键填入，回复关键词着色 |
| 需要可视化传感器数据统计趋势 | **数据仪表盘**，自动提取数值并统计 Min/Max/Avg |
| 中英文用户使用同一工具 | **中英双语界面**，全局菜单栏一键切换，即时生效 |
...
| **CRC 助手** | CRC-8/16/32 + SUM-8/16 五种算法实时计算，HEX 输入即出结果，支持追加到发送区 |
| **AT 指令伴侣** | 内置 10+ 常用 AT 指令模板库，一键填入发送区，回复 OK/ERROR 关键词着色 |
| **数据仪表盘** | 复用波形图正则规则，数值卡片展示最新值 + Min/Max/Avg 统计 |
| **端口配置记忆** | 自动保存每个端口的波特率/数据位/校验位，下次连接自动恢复 |
| **中英双语界面** | 全部界面文本支持中英文切换，全局菜单栏设置，即时生效 |
...
│   ├── preset/                        # 指令预设管理
│   ├── at/                            # AT 指令伴侣（模板库 + 回复着色）
│   ├── dashboard/                     # 数据仪表盘（数值卡片）
│   ├── crc/                           # CRC 校验计算面板
│   ├── config/                        # 端口配置记忆持久化
│   └── i18n/                          # 国际化（LocaleManager, Messages, 资源文件）
...
- [x] **v0.4** — CRC 助手、端口配置记忆、AT 指令伴侣、数据仪表盘、中英双语国际化
